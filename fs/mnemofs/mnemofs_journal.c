...
int mfs_jrnl_fmt(FAR struct mfs_sb_info * const sb)
{
  ...
  mfs_t wr_s_blkidx;                  /* idxarr[wr_s_blkidx] is start of
                                       * writeable area
                                       */
  mfs_t *idxarr        = NULL;
  char *buf            = NULL;
                  ...
                  mfs_t *m_node)
{
  ...
  if (predict_true(sb->blk_sz >= buf_sz))
  ...
  else
    {
      i                  = 0;
      tmp                = 0;
      tmp                += mfs_read_data(sb, buf, sb->blk_sz,
                                          MFS_BLK2PG(sb, blk), 0);
      sb->j_state.idxarr = (mfs_t *) (buf + 8);
      i++;
      while (tmp < buf_sz)
  ...
  if (predict_true(info != NULL))
    {
      ...
      memcpy(&info->depth, buf + MFS_JLOG_DEPTH_OFF, sizeof(info->depth));
      memcpy(&info->path, buf + MFS_JLOG_PATH_OFF,
              info->depth * sizeof(struct mfs_ctz_store_s));
      memcpy(&info->new, buf + MFS_JLOG_NEW_OFF(info), sizeof(info->new));
                  ...
                  FAR const struct mfs_jrnl_info * const info)
{
  ...
  memcpy(buf + MFS_JLOG_PATH_OFF, info->path,
        sizeof(info->path) * info->depth);
  memcpy(buf + MFS_JLOG_NEW_OFF(info), &info->new, sizeof(info->new));
  hash = mfs_arrhash(buf, MFS_JLOG_HASH_OFF(info));
